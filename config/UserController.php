...
class UserController extends Controller
{
    ...
    public function index($json=false)
    {
        ...
        if(empty($userRecord->img)){
             ...
             $profilePic = URL::to("userimages/user_".$this->userId."/medium/".$userRecord->img);
         }
         
         if(empty($userRecord->background_img)){
             $coverPic = URL::to("images/user.png");
         }else{
             $coverPic = URL::to("userimages/user_".$this->userId."/profile_background/".$userRecord->background_img);
         }
   ...
   public function showProfilePic()
   {
    ...
    {
        //$userProfileCover=URL::to('/userimages/user_'.$this->userId.'/profile_background/');
        //$userProfilePic=URL::to('/userimages/user_'.$this->userId.'/medium/');
        $userInfo=userProfile::where('uid',$this->userId)
                 ->first();
        $userProfileCover=URL::to('/userimages/user_'.$this->userId.'/profile_background/'.$userInfo->background_img);
        $userProfilePic=URL::to('/userimages/user_'.$this->userId.'/medium/'.$userInfo->img);
        if($userInfo->img)
